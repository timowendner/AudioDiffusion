...
class Diffusion(nn.Module):
    ...
    def sample(self, model, config):
        ...
        labels = [config.create_label + 1] * n # TODO
        model.eval()

        # create a noise array that we want to denoise
        x = torch.randn(n, 1, self.length, device=device)
        l = torch.tensor(labels, device=device).view(-1, 1)
        ...
        for i in range(1, self.steps):
            for _ in range(config['create_loop']):
                ...
                t = torch.ones(n, device=device).long() * \
                    (self.steps - i)
